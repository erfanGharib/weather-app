...
class WeatherProperties extends Component {
    ...
    render() {
        const {tempF, tempC, weatherText, humidity, windSpeed, time, date_, onDayChange} = this.props;
        return (
            <div className='w-full h-full flex-col flex justify-between items-center'>
                <div className='flex flex-col justify-center items-center w-full'>
                    ...
                    <span id='weather-text' className='capitalize font-medium text-3xl'>{weatherText}</span>
                ...
                <ul
                    id='day-of-week'
                    className='relative text-opacity-90 flex flex-col-reverse my-auto items-center overflow-hidden'
                >
                    {this.state.days.map((value,li)=>
                        <li 
                            key={li}
                            ref={value}
                            onClick={()=>onDayChange(li)}
                            className='cursor-pointer capitalize font-bold text-3xl opacity-50'
                        >
                            {dayNames[this.state.currentDay+li]}
                        </li>
                    )}
                </ul>
            </div>
